...
""" This is a ROS node written in Python which is
    subscribing to ROS topic "ar_pose_marker_1" of ROS package
    (ar_track_alvar/AlvarMarkers) and publishing the topic multiple_pose_marker_1 under the message type AlvarMarkers
...
# Calculates Pose and orientation of an all individual Ar tags according to their marker_id
    for i in range(0, marker_number):
        ...
        rospy.loginfo("frame ID of detected marker = %s", req.markers[i].header.frame_id)
...
# The callback argument calls the function which is defined earlier
    sub = rospy.Subscriber('ar_pose_marker_1', AlvarMarkers, callback)

# Declaration of node publishing to the "multiple_pose_marker_1" topic using the message type "AlvarMarkers"
# The queue_size argument limits the amount of queued messages if any subscriber is not receiving them fast enough.
    pub = rospy.Publisher('multiple_pose_marker_1', AlvarMarkers, queue_size=10)
